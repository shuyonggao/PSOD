...
class LocalSaliencyCoherence(torch.nn.Module):
    ...
    def _create_kernels_from_features(features, radius):
        ...
        kernels = kernels - kernels[:, :, radius, radius, :, :].view(N, C, 1, 1, H, W) # 以中心点作为一个基准
        kernels = (-0.5 * kernels ** 2).sum(dim=1, keepdim=True).exp() #  核函数，在隐空间中的内积
    ...
    def _unfold(img, radius):
        ...
        N, C, H, W = img.shape
        diameter = 2 * radius + 1
        return F.unfold(img, diameter, 1, radius).view(N, C, diameter, diameter, H, W)  # -> (N, C*K_h*k_w, 滑了多少次).view(N,C,K_h,K_w,H,W)
